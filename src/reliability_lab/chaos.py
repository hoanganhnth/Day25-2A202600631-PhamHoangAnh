    ...
    for breaker in gateway.breakers.values():
        ...
        for transition in breaker.transition_log:
            if transition["to"] == "open":
                open_ts = transition["ts"]
            elif transition["to"] == "closed" and open_ts is not None:
                recovery_times.append((transition["ts"] - open_ts) * 1000)
                open_ts = None
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        if "timeout" in scenario.name.lower() or "failure" in scenario.name.lower():
            fallback_success_rate = result.fallback_successes / max(1, result.fallback_successes + result.failed_requests)
            passed = fallback_success_rate >= 0.5
        else:
            passed = (result.successful_requests / max(1, result.total_requests)) >= 0.9
