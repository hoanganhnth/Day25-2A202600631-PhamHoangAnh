...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        ...
        def tokenize(s: str) -> list[str]:
            words = s.lower().split()
            ngrams = [word[i:i+3] for word in words for i in range(len(word)-2)]
            return words + ngrams

        counts_a = Counter(tokenize(a))
        counts_b = Counter(tokenize(b))

        dot_product = sum(counts_a[k] * counts_b[k] for k in counts_a.keys() & counts_b.keys())
        mag_a = math.sqrt(sum(v**2 for v in counts_a.values()))
        mag_b = math.sqrt(sum(v**2 for v in counts_b.values()))
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        if best_score >= self.similarity_threshold and best_query and best_response:
            if _looks_like_false_hit(query, best_query):
                self.false_hit_log.append({"query": query, "cached_key": best_query, "score": best_score, "reason": "date_or_number_mismatch"})
                return None, best_score
